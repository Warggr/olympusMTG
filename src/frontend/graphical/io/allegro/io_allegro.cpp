...
inline void al_draw_scaled_bitmap(ALLEGRO_BITMAP* bitmap, int picY, int picZ, int picW, int picH, const Rect& zone, char flags){
...
void AllegroIO::disp_mana(Mana mana, int endy, int topz) const {
    ...
    for(int i=Mana::xpos::FIRST_COLOR; i<=Mana::xpos::COLORLESS; ++i){
        for(uint j=0; j < ex[i]; ++j){
            endy -= 24;
            al_draw_scaled_bitmap(ManaColorSym[i], 0, 0, 160, 160, endy, topz, 24, 24, 0);
        }
    }
    if(ex[Mana::xpos::GENERIC]!=0 || mana.cmc() == 0){
        al_draw_scaled_bitmap(ManaNumSym[ex[Mana::xpos::GENERIC]], 0, 0, 160, 160, endy-24, topz, 24, 24, 0);
    }
}
...
void AllegroIO::disp_inrow(const Displayable* disp, int number, int total, int flags) const {
    ...
    int stepsize = width / total;
    AllegroCanvas canvas(this, Rect(0 + stepsize * number, 0, stepsize, UNLIMITED));
    disp->disp(&canvas, flags);
}

void AllegroIO::draw_permanent(const Rect& zone, const std::string& name, char color, bool tapped, disp_flags flags, bool basicImg) const {
    ...
    const int lw = 10;
    if(flags) al_draw_rectangle(zone.y+lw/2, zone.z+lw/2, zone.y+zone.width-lw/2, zone.z+zone.height-lw/2, registeredColors[AbstractIO::HIGH2], lw);
}
...
void AllegroIO::draw(const Permanent& perm, const Rect& zone, disp_flags flags) const {
    char color = main_color(perm.getManaCost().m2color());
    draw_full_rectangle(color, zone);
    draw_boxed_text(perm.getName(), flags ? HIGH1 : BLACK, flags ? BLACK : WHITE, zone.y, zone.z, zone.width);
}

void AllegroIO::draw(const Resolvable& resolvable, const Rect& zone, disp_flags flags) const {
    draw_full_rectangle(GREY, zone);
    draw_boxed_text(resolvable.getName(), flags ? HIGH1 : BLACK, flags ? BLACK : WHITE, zone.y, zone.z, zone.width);
}

void AllegroIO::draw(const Gamer& player, const Rect& zone, disp_flags flags) const {
    int x = flags ? HIGH2 : WHITE;
...
void AllegroIO::draw(const CardOracle& card, const Rect& rect, disp_flags flags) const {
    (void) flags; //TODO
