...
void PermOption::fillout(Mana c, int nbtars, char* typesoftars, Ability* f, bool ts, bool ismana){
	cost = c; nbtargets = nbtars; typesoftargets = typesoftars, fab = f, tapsymbol = ts, ismanaability = ismana;
}
...
void Player::draw(int nb_cards){
	...
	for(int i=0; i<nb_cards; i++){
		...
		int x = (int) iter->get_type();
		if(x>2) x = 2;
		myzones[0].set_first(iter->next); //remove from lib
		Card* i2 = iter;
		if(x==1) myoptions[LANDOPTS] = new PlayLand(iter, this);
		else{
			first_spell = new SpellOption(iter, first_spell);
			nb_spells++;
		}
		iter = iter->next;
		i2->eject();
