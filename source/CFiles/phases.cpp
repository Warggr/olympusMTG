...
bool Player::turn(){
	...
	for(auto iter = thisTurnsOrder.begin(); iter != thisTurnsOrder.end(); iter++){
...
void Player::untapstep(){
	...
	nb_mainphase = 0;
	auto canAttackFunct = [](Permanent& p){p.set_flags(2); };
	for_each_perm(canAttackFunct);
	auto untapFunct = [](Permanent& p){p.untap(); };
	for_each_perm(untapFunct);
	metagame->disp();
}
